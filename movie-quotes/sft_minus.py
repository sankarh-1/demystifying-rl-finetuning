...
def create_target_dataset(tokenizer):
    ...
    messages = [
        ...
        {"role": "assistant", "content": f"quote: {config.TARGET_QUOTE}{eos}"}
    ]
...
def train_sft_minus():
    ...
    print(f"=== [PHASE 3] SFT- Training ({model_str}) ===")
    out_dir = config.MODELS_DIR / "sft_minus"
    if out_dir.exists(): shutil.rmtree(out_dir, ignore_errors=True)

    try: tokenizer = AutoTokenizer.from_pretrained(config.MODEL_NAME, local_files_only=True)
    ...
    try: model = AutoModelForCausalLM.from_pretrained(config.MODEL_NAME, torch_dtype=torch.bfloat16, device_map="auto", local_files_only=True)
    except: model = AutoModelForCausalLM.from_pretrained(config.MODEL_NAME, torch_dtype=torch.bfloat16, device_map="auto")
    ...
    args = TrainingArguments(
        ...
        learning_rate=config.SFT_MINUS_LR, save_strategy="no", logging_steps=10, bf16=True,
        report_to="wandb", run_name=f"sft_minus_{model_str}"
    )
    ...
    merged_model.save_pretrained(str(out_dir / "final"))
