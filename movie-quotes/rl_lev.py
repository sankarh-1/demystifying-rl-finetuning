...
def train_rl_lev(source_mode):
    model_str = config.MODEL_NAME.split('/')[-1]
    run_name = f"rl_lev_{source_mode}_{model_str}"
    print(f"=== [PHASE 6] RL Lev Training on {source_mode} ===")
    
    if source_mode == "base": pretrain_path = config.MODEL_NAME
    else: pretrain_path = str(config.MODELS_DIR / source_mode / "final")
        
    save_path = str(config.MODELS_DIR / f"rl_lev_{source_mode}" / "final")
    ...
    cmd = [
        ...
        "--remote_rm_url", "http://localhost:5000/get_reward_lev",
