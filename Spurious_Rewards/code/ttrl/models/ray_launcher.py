...
from ttrl.models.model import get_llm_for_sequence_regression
...
class PPORayActorGroup:
    ...
    def async_init_actor_trainer(
        ...
        """
        Init trainer for each actor worker
    ...
    def async_fit_actor_model(self, steps, shared_data_refs, pretrain_dataset=None, wait_for_refs=None):
        """Train actor model.
        Returns:
            List: list of remote object refs.
        """
        refs = []
        for i, actor in enumerate(self._actor_handlers):
            refs.append(actor.fit.remote(steps, shared_data_refs[i], pretrain_dataset, wait_for_refs))

        return refs

    def async_init_prime_trainer(
            self,initial_model_group: "PPORayActorGroup",
        ):
        """
        Init trainer for each prime worker
        """
        initial_actors = initial_model_group._actor_handlers
    ...
    def async_fit_prime_model(self, steps, prime_samples):
        """Train actor model.
        Returns:
            List: list of remote object refs.
        """
        refs = []
        for i, actor in enumerate(self._actor_handlers):
            refs.append(actor.fit.remote(steps, prime_samples))

        return refs

    def async_fit_and_reward_prime_model(self, steps, prime_samples):
        """Train actor model.
        Returns:
            List: list of remote object refs.
        """
        refs = []
        for i, actor in enumerate(self._actor_handlers):
            refs.append(actor.fit_and_reward.remote(steps, prime_samples))
