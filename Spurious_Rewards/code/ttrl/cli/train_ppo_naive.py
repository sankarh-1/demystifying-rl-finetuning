...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_entropy_loss", action="store_true",
                        default=False, help="Use entropy loss")
    parser.add_argument("--use_policy_loss_wo_old_log_probs", action="store_true",
                        default=False, help="Use policy loss without old log probs")
    parser.add_argument("--entropy_coef", type=float, default=0.0,
                        help="Entropy coefficient for PPO")
    parser.add_argument("--use_online_sft", action="store_true",
                        default=False, help="Use online SFT")
    parser.add_argument("--actor_learning_rate", type=float, default=1e-6)
    parser.add_argument("--actor_scheduler", type=str, default="cosine_with_min_lr")
    ...
    parser.add_argument("--verify_task", type=str, default="math", help="which verify to used for the task")
    parser.add_argument("--verify_task_eval", type=str, default="math", choices=[
                        "math", "simplerl", "review", "review_group", "think", "gpqa", "medxpertqa", "math_r1_style", "dense"], help="which verify to used for the task")
    parser.add_argument("--filter_samples_by_reward",
                        action="store_true", default=False)
    ...
    parser.add_argument("--extra_eval_task", type=str, default=None, help="extra eval task")
    parser.add_argument("--extra_eval_task_fast", type=str, default=None, help="extra eval task")
    parser.add_argument("--extra_eval_task_fast_supress_orig_eval", action="store_true", default=False, help="supress original eval task")
    # parser.add_argument("--extra_eval_max_orig_dataset_count", type=int, default=100, help="max count of original dataset for extra eval task")
    # parser.add_argument("--extra_eval_temperature_of_orig_dataset", type=float, default=1, help="temperature for evaluation of original dataset for extra eval task")
    parser.add_argument("--extra_eval_trajectory", type=str, default=None, help="extra eval trajectory")
    parser.add_argument("--extra_eval_trajectory_steps", type=int, default=None, help="extra eval trajectory steps")
    parser.add_argument("--extra_eval_trajectory_temperature", type=float, default=1, help="extra eval trajectory temperature")
    parser.add_argument("--use_golden_response", action="store_true", default=False, help="use golden response")
    parser.add_argument("--n_votes_per_prompt", type=int, default=None, help="number of votes per prompt")
    parser.add_argument("--filter_samples_by_majority_ratio", type=float, default=None, help="filter samples by majority ratio")
    parser.add_argument("--eval_temperature", type=float, default=0, help="temperature for evaluation")
    parser.add_argument("--eval_temperature_at_k", type=float, default=0.6, help="temperature for evaluation at k")
    parser.add_argument("--eval_top_p", type=float, default=1.0, help="top_p for evaluation")
    ...
    tasks = ["math", "simplerl", "review", "review_group", "box_only_format_r1_style", "no_repetition",
                                             "think", "ttt", "gpqa", "medxpertqa", "format",
                                             "math_r1_style", "dense", "box_only_format",
                                             "inverse_box_only_format", "inverse_math", "inverse_math_r1_style", "contain_python", "contain_python_wo_backticks"]

    def verify_task_valid(task):
        if "_AND_" in task:
            specified_tasks = task.split("_AND_")
            return len(specified_tasks) >= 2 and all([verify_task_valid(t) for t in specified_tasks])
        canonical_task_name = task.replace("inverse_", "") if task.startswith("inverse_") else task
        canonical_task_name = canonical_task_name.replace("_r1_style", "") if canonical_task_name.endswith("_r1_style") else canonical_task_name
        return (canonical_task_name in tasks) or \
            (canonical_task_name.startswith("random") and 0 <= float(canonical_task_name.replace("random", "")) <= 1)

    assert verify_task_valid(args.verify_task), "verify_task must be math or random"

    assert not (args.use_online_sft and args.use_entropy_loss), "use_online_sft and use_entropy_loss cannot be True simultaneously"
    
    if not args.extra_eval_task_fast:
        args.extra_eval_task_fast_supress_orig_eval = False
