...
def extract_answer_box_only(pred_str, data_name, use_last_number=True, is_r1_style=False):
    pred_str = pred_str.replace("\u043a\u0438", "")
    if data_name in ["mmlu_stem", "sat_math", "aqua", "gaokao2023"]:
        # TODO check multiple choice
        return choice_answer_clean(pred_str)
    elif data_name in ["gpqa"]:
        return choice_answer_clean_gpqa(pred_str)
    elif data_name in ["medxpertqa"]:
        return choice_answer_clean_medxpertqa(pred_str)

    if is_r1_style:
        pred_str = pred_str.split("<answer>")[-1].rstrip("</answer>")
        return pred_str
...
def extract_answer(pred_str, data_name, use_last_number=True, is_r1_style=False):
    pred_str = pred_str.replace("\u043a\u0438", "")
    if data_name in ["mmlu_stem", "sat_math", "aqua", "gaokao2023"]:
        # TODO check multiple choice
        return choice_answer_clean(pred_str)
...
def choice_answer_clean_gpqa(pred: str):
    pred = pred.strip("\n").rstrip(".").rstrip("/").strip(" ").lstrip(":")
    # Clean the answer based on the dataset
...
def choice_answer_clean_medxpertqa(pred: str):
    pred = pred.strip("\n").rstrip(".").rstrip("/").strip(" ").lstrip(":")
    # Clean the answer based on the dataset
